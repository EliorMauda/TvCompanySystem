...
int isComplaintAccepted(const TvCompany* pCompany, const Complaints* cpmplaintArr, const Channel* userChannel)
{
	...
	if (!(userChannel->CommercialArr) && (numOfComplaintType == 0 || numOfComplaintType == 2))
	{
		printf("No commercials have been added to the system yet.");
		return 0;
	}
	else if (!(userChannel->showsList.head.next) && numOfComplaintType == 1)
	{
...
int RespondingToCustomerComplaints(TvCompany* pCompany, Complaints* pComp, Channel* userChannel)
{
	...
	case 2:
	{
		...
		{
			printf("The commercial has been deleted at your request.\n");
		}
...
void sortViewer(TvCompany* pCompany)
{
	...
	{
		printf("There are no at least 2 viewers in the company that can be sorted.\n");
	}
}
...
void writeCompanyToFile(const TvCompany* company, const char* filename)
{
	...
	//Write sort type
	fprintf(file, "%d\n", company->sortType);// Write the name
...
void writeCompanyToBinaryFile(const TvCompany* company, const char* filename)
{
	...
	fwrite(&company->sortType, sizeof(int), 1, file); // Write age
...
TvCompany readCompanyFromBinaryFile(const char* filename)
{
	...
	int sortType;
	fread(&sortType, sizeof(int), 1, file);
